...
def write_c_header_file(file, apis, public_header=False):
  if not public_header:
    ...
    for api in apis:
      if api['type'] in [ 'struct', 'refcounted', 'class', 'enum', 'enum class' ]:
        if api['name'].startswith('gfx::') or \
           api['name'].startswith('Sk') or \
           '::' in api['name']:
...
def write_converters(file, apis, api):
  ...
  if api['type'] == 'struct':
    ...
    return_enum_classes = [
        ...
        f'    return {get_cpp_enum_name(api["name"], enum["name"])};\n' for enum in api['enums'] ]
    write_function(file, api, True,
                   f'inline {api_cpp_type} ToHime',
                   [ f'{api_type_name} c' ],
    ...
    # Write a converter from cpp to c.
    return_enums = [
        f'  case {get_cpp_enum_name(api["name"], enum["name"])}:\n'
...
def get_cpp_enum_name(api_name, enum_name):
  if api_name == 'ColorId':
    return 'ui::kColor' + enum_name
  return api_name + '::k' + enum_name
...
def get_c_arg(apis, arg, prefix=''):
  ...
  if arg['type'].startswith('std::vector<'):
    if arg_type == 'struct':
      return f'ToHimeVector<{arg["type"][12:-1]}>({arg_name}, {arg_name}_size)'
    else:
